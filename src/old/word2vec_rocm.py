...
def sentence_vector(sentence, model, word_to_idx):
    words = simple_preprocess(sentence)
    indices = [word_to_idx[word] for word in words if word in word_to_idx]
    if not indices:
        return np.zeros(model.embeddings.embedding_dim).tolist()  # Return a zero vector if no words are found
    embeddings = model(torch.tensor(indices, dtype=torch.long).to(next(model.parameters()).device)).detach().cpu().numpy()
...
def main():
    ...
    embeddings = [sentence_vector(sentence, model, word_to_idx) for sentence in df['sentence']]
